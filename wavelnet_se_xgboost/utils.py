...
def get_windows(signal, window_size, method='random', num_windows=1, stride=None, qrs_offset=None):
    ...
    if method == 'random':
        ...
        raise NotImplementedError(f"Unknown method '{method}'. Choose from 'random', 'tiled', 'qrs', or 'entire_recording'.")
...
def compute_challenge_score(labels, outputs, fraction_capacity = 0.05, num_permutations = 10**4, seed=12345):
    ...
    return tpr
def generate_k_folds(k_folds, records):
    """
    Generate k folds for cross-validation based on the 'label' key.
    Each fold is a list of record dicts.
    """
    if k_folds <= 0 or k_folds > len(records):
        raise ValueError("k_folds must be a positive integer less than or equal to the number of records.")
    ...
    folds = []
    for _, test_idx in skf.split(records, labels):
