...
def train_finetune_model(data_folder, model_folder, verbose):
    ...
    trainer = pl.Trainer(
        ...
        enable_checkpointing=False  # No sanity check, disables validation/test,
    )

    trainer.fit(fine_tune_module, datamodule=data_module)
    # Save the final fine-tuned model
    # DO NOT REMOVE THE INITIAL ENCODER, IT IS NEEDED TO LOAD THE MODEL FOR PREDICTION!!
    # for file in os.listdir(model_folder):
    #     if file.endswith('.ckpt') or file.endswith('.pth'):
    #         print(f"Removing existing model file: {file}")
    #         os.remove(os.path.join(model_folder, file))
    final_model_path = os.path.join(model_folder, 'finetuned_model.ckpt')
