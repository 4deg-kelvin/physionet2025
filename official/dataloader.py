...
class ECGWindowAlignedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # # Check if all segments are of the same length
        # assert np.all([seg.shape[-1] == self.window for seg in ecg_segs]), "Not all segments are of the same length"
        # assert len(ecg_segs) == self.nb_windows
        return ecg_segs, feats_normalized, lbl, hdr, "x"
...
class ECGWindowPaddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ecg_segs = np.array([data[:,start:start+self.window] for start in starts])
        assert ecg_segs[0].shape[-1] == self.window
        return ecg_segs, feats_normalized, lbl, hdr, 'x'        
...
def load_challenge_data(header_file):
    ...
    # print("original recording shape", recording.shape)
    ...
    # print("resampled recording shape", recording.shape)
    return recording, header
def load_challenge_data_dat(header_file):
    with open(header_file, 'r') as f:
        header = f.readlines()    
    sampling_rate = int(header[0].split()[2]) 
    dat_file = header_file.replace('.hea', '.dat')
    signal = load_signals(dat_file.replace(".dat", ""))
    x = signal[0]
    header = signal[1]
    recording = x.astype(np.float64).T
    # print("orig sampling rate:", sampling_rate)
    # print("original recording shape", recording.shape)
    
    # Standardize sampling rate
    if sampling_rate > fs:
        recording = decimate(recording, int(sampling_rate / fs))
        # print("decimated recording shape", recording.shape)
    elif sampling_rate < fs:
        recording = resample(recording, int(recording.shape[-1] * (fs / sampling_rate)), axis=1)
    #     print("resampled recording shape", recording.shape)
    # print("fs:", fs)
    # print("final recording shape", recording.shape)
    return recording, header
