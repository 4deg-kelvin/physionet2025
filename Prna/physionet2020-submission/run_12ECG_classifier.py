...
def run_12ECG_classifier_batch(data_batch, header_data_batch, loaded_model):
    ...
    # Stack the processed inputs for batch prediction
    inp_t_batch = torch.stack(processed_recordings)
    feats_t_batch = torch.stack(all_feats_t)
...
def predict_batch(models, thrs, inp_windows_t_batch, feats_t_batch, hdr_batch, device=device):
    '''
    Predict using the loaded models for a batch of inputs
    models: list of pytorch model
    thrs: list of individual model thresholds
    inp_windows_t_batch: torch tensor holding random recording windows for the batch [batch_size x nb_windows x 12 x window_size]
    feats_t_batch: torch tensor holding wide features for the batch [batch_size x nb_feats]
    hdr_batch: list of header file contents for the batch
    device: cpu or gpu
    returns, batch_probs: numpy arr of probs per class for the batch [batch_size x nb_classes]
             batch_preds: numpy arr of preds per class for the batch [batch_size x nb_classes]
    '''
    batch_size = inp_windows_t_batch.shape[0]
    batch_probs_list, batch_preds_list = [], []

    mean_age = pd.read_csv('mean_age.csv', index_col=0).age.values[0]
    std_age = pd.read_csv('std_age.csv', index_col=0).age.values[0]

    # Prepare demographic data for the batch
    age_t_list = []
    sex_t_list = []
    for hdr in hdr_batch:
        age_t_list.append((get_age(hdr[13])[None].T - mean_age) / std_age)
        sex_t_list.append([1. if hdr[14].find('Female') >= 0. else 0.])
    
    age_t_batch = torch.FloatTensor(np.array(age_t_list)).squeeze(dim=2).to(device) # [batch_size x 1]
    sex_t_batch = torch.FloatTensor(sex_t_list).to(device) # [batch_size x 1]

    # Ensure feats_t_batch is correctly shaped: [batch_size x nb_feats]
    # If feats_t_batch was [batch_size x 1 x nb_feats], squeeze it.
    if len(feats_t_batch.shape) == 3 and feats_t_batch.shape[1] == 1:
        feats_t_batch = feats_t_batch.squeeze(dim=1)

    feats_t_batch = feats_t_batch.to(device) # [batch_size x nb_feats]
    wide_feats_batch = torch.cat([age_t_batch, sex_t_batch, feats_t_batch.float()], dim=1).to(device) # [batch_size x (2 + nb_feats)]
    ...
    for model, thr in zip(models, thrs):
        ...
        fold_preds_list = []
        with torch.no_grad():
            # inp_windows_t_batch is [batch_size, nb_windows, num_channels, window_size]
            # We want to process each window across the batch, then average results per input item.
            # Transpose to [nb_windows, batch_size, num_channels, window_size]
            inp_windows_t_batch_transposed = inp_windows_t_batch.transpose(0, 1).float().to(device)
            
            outs_across_windows = [] # To store outputs for each window, for all items in batch
            for inp_t_window_batch in inp_windows_t_batch_transposed:
                # inp_t_window_batch is [batch_size, num_channels, window_size]
                # wide_feats_batch is [batch_size, nb_demo + nb_feats]
                out_window_batch = model(inp_t_window_batch, wide_feats_batch) # [batch_size, num_classes]
                outs_across_windows.append(out_window_batch)
            
            # Stack and average over windows: [nb_windows, batch_size, num_classes] -> [batch_size, num_classes]
            out_batch_avg = torch.stack(outs_across_windows).mean(dim=0)

            prob_batch = out_batch_avg.sigmoid().data.cpu().numpy() # [batch_size, num_classes]
            fold_probs_list.append(prob_batch)
            pred_batch = prob_batch > thr # [batch_size, num_classes]
            fold_preds_list.append(pred_batch)
        
        batch_probs_list.append(np.stack(fold_probs_list).mean(axis=0)) # Average probs over folds for this model, then append
        batch_preds_list.append(np.any(np.stack(fold_preds_list), axis=0)) # Take 'any' for preds over folds for this model

    # Consolidate probs and preds across models (folds are already handled)
    # If multiple models (different fold checkpoints) are used as an ensemble:
    final_batch_probs = np.mean(batch_probs_list, axis=0) # Average probabilities across different model checkpoints
    final_batch_preds = np.any(batch_preds_list, axis=0)   # Take 'any' across different model checkpoints

    return final_batch_probs, final_batch_preds
